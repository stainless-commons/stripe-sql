...
ALTER TYPE stripe_minimal_coupon.coupon_list_response
  ADD ATTRIBUTE "data" stripe_minimal_coupon.coupon[],
  ADD ATTRIBUTE has_more BOOLEAN,
  ADD ATTRIBUTE "object" TEXT,
  ADD ATTRIBUTE url TEXT;

CREATE OR REPLACE FUNCTION stripe_minimal_coupon.make_coupon_list_response(
  "data" stripe_minimal_coupon.coupon[],
  has_more BOOLEAN,
  "object" TEXT,
  url TEXT
)
RETURNS stripe_minimal_coupon.coupon_list_response
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT ROW(
    "data", has_more, "object", url
  )::stripe_minimal_coupon.coupon_list_response;
$$;
...
CREATE OR REPLACE FUNCTION stripe_minimal_coupon._list(
...
AS $$
  ...
  response = GD["__stripe_minimal_context__"].client.coupons.with_raw_response.list(
...
AS $$
  BEGIN
    PERFORM stripe_minimal_internal.ensure_context();
    RETURN jsonb_populate_record(
      NULL::stripe_minimal_coupon.coupon_list_response,
      stripe_minimal_coupon._list(
