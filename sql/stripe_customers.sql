...
IMMUTABLE
AS $$
  SELECT ROW(country, source, state)::stripe_customers.customer_tax_location;
$$;
...
AS $$
  ...
  response = GD["__stripe_context__"].client.customers.with_raw_response.create(
      ...
      cash_balance=not_given if cash_balance is None else GD["__stripe_context__"].strip_none(cash_balance),
      ...
      tax=not_given if tax is None else GD["__stripe_context__"].strip_none(tax),
      tax_exempt=not_given if tax_exempt is None else tax_exempt,
      tax_id_data=not_given if tax_id_data is None else GD["__stripe_context__"].strip_none(tax_id_data),
...
AS $$
  ...
  page = GD["__stripe_context__"].client.customers.list(
