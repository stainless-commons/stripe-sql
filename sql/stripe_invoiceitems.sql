ALTER TYPE stripe_invoiceitems.invoiceitem_create_response
  ...
  ADD ATTRIBUTE parent stripe_invoiceitems.invoiceitem_create_response_parent,
  ADD ATTRIBUTE pricing stripe_invoiceitems.invoiceitem_create_response_pricing,
  ADD ATTRIBUTE proration_details stripe_invoiceitems.invoiceitem_create_response_proration_detail,
...
CREATE OR REPLACE FUNCTION stripe_invoiceitems.make_invoiceitem_create_response(
  ...
  parent stripe_invoiceitems.invoiceitem_create_response_parent DEFAULT NULL,
  pricing stripe_invoiceitems.invoiceitem_create_response_pricing DEFAULT NULL,
  proration_details stripe_invoiceitems.invoiceitem_create_response_proration_detail DEFAULT NULL,
  tax_rates stripe_invoices.tax_rate[] DEFAULT NULL,
  test_clock JSONB DEFAULT NULL
)
RETURNS stripe_invoiceitems.invoiceitem_create_response
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT ROW(
    ...
    tax_rates,
    test_clock
  )::stripe_invoiceitems.invoiceitem_create_response;
$$;
...
RETURNS stripe_invoiceitems.invoiceitem_create_response_pricing
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT ROW(
...
ALTER TYPE stripe_invoiceitems.invoiceitem_create_response_pricing_price_detail
  ADD ATTRIBUTE price JSONB, ADD ATTRIBUTE product TEXT;

CREATE OR REPLACE FUNCTION stripe_invoiceitems.make_invoiceitem_create_response_pricing_price_detail(
  price JSONB, product TEXT
)
RETURNS stripe_invoiceitems.invoiceitem_create_response_pricing_price_detail
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT ROW(
    price, product
  )::stripe_invoiceitems.invoiceitem_create_response_pricing_price_detail;
$$;

ALTER TYPE stripe_invoiceitems.invoiceitem_create_response_proration_detail
  ADD ATTRIBUTE discount_amounts stripe_invoices.discounts_resource_discount_amount[];

CREATE OR REPLACE FUNCTION stripe_invoiceitems.make_invoiceitem_create_response_proration_detail(
  discount_amounts stripe_invoices.discounts_resource_discount_amount[]
)
RETURNS stripe_invoiceitems.invoiceitem_create_response_proration_detail
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT ROW(
...
CREATE OR REPLACE FUNCTION stripe_invoiceitems.create(
  amount BIGINT DEFAULT NULL,
  currency TEXT DEFAULT NULL,
  customer TEXT DEFAULT NULL,
  customer_account TEXT DEFAULT NULL,
...
AS $$
  BEGIN
    ...
    RETURN jsonb_populate_record(
      NULL::stripe_invoiceitems.invoiceitem_create_response,
      stripe_invoiceitems._create(
        amount,
        currency,
        customer,
        customer_account,
