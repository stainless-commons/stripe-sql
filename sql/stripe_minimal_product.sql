ALTER TYPE stripe_minimal_product.product
  ADD ATTRIBUTE "id" TEXT,
  ADD ATTRIBUTE active BOOLEAN,
  ADD ATTRIBUTE created BIGINT,
  ADD ATTRIBUTE images TEXT[],
  ADD ATTRIBUTE livemode BOOLEAN,
  ADD ATTRIBUTE marketing_features stripe_minimal_product.product_marketing_feature[],
  ...
  ADD ATTRIBUTE package_dimensions stripe_minimal_product.product_package_dimension,
...
CREATE OR REPLACE FUNCTION stripe_minimal_product.make_product(
  "id" TEXT,
  active BOOLEAN,
  created BIGINT,
  images TEXT[],
  livemode BOOLEAN,
  marketing_features stripe_minimal_product.product_marketing_feature[],
  metadata JSONB,
  "name" TEXT,
  "object" TEXT,
  updated BIGINT,
  ...
  package_dimensions stripe_minimal_product.product_package_dimension DEFAULT NULL,
...
RETURNS stripe_minimal_product.product
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT ROW(
    ...
    unit_label,
    url
  )::stripe_minimal_product.product;
$$;

ALTER TYPE stripe_minimal_product.product_marketing_feature
  ADD ATTRIBUTE "name" TEXT;

CREATE OR REPLACE FUNCTION stripe_minimal_product.make_product_marketing_feature(
  "name" TEXT DEFAULT NULL
)
RETURNS stripe_minimal_product.product_marketing_feature
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT ROW("name")::stripe_minimal_product.product_marketing_feature;
$$;

ALTER TYPE stripe_minimal_product.product_package_dimension
...
CREATE OR REPLACE FUNCTION stripe_minimal_product.make_product_package_dimension(
  height DOUBLE PRECISION,
  "length" DOUBLE PRECISION,
  weight DOUBLE PRECISION,
  width DOUBLE PRECISION
)
RETURNS stripe_minimal_product.product_package_dimension
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT ROW(
...
ALTER TYPE stripe_minimal_product.default_price_data
  ...
  ADD ATTRIBUTE custom_unit_amount stripe_minimal_product.default_price_data_custom_unit_amount,
  ADD ATTRIBUTE metadata JSONB,
  ADD ATTRIBUTE recurring stripe_minimal_product.default_price_data_recurring,
  ADD ATTRIBUTE tax_behavior TEXT,
  ADD ATTRIBUTE unit_amount BIGINT,
  ADD ATTRIBUTE unit_amount_decimal TEXT;

CREATE OR REPLACE FUNCTION stripe_minimal_product.make_default_price_data(
  currency TEXT,
  currency_options JSONB DEFAULT NULL,
  custom_unit_amount stripe_minimal_product.default_price_data_custom_unit_amount DEFAULT NULL,
  metadata JSONB DEFAULT NULL,
  recurring stripe_minimal_product.default_price_data_recurring DEFAULT NULL,
  tax_behavior TEXT DEFAULT NULL,
  unit_amount BIGINT DEFAULT NULL,
  unit_amount_decimal TEXT DEFAULT NULL
)
RETURNS stripe_minimal_product.default_price_data
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT ROW(
    ...
    unit_amount,
    unit_amount_decimal
  )::stripe_minimal_product.default_price_data;
$$;

ALTER TYPE stripe_minimal_product.default_price_data_custom_unit_amount
  ADD ATTRIBUTE enabled BOOLEAN,
  ADD ATTRIBUTE maximum BIGINT,
  ADD ATTRIBUTE minimum BIGINT,
  ADD ATTRIBUTE preset BIGINT;

CREATE OR REPLACE FUNCTION stripe_minimal_product.make_default_price_data_custom_unit_amount(
  enabled BOOLEAN,
  maximum BIGINT DEFAULT NULL,
  minimum BIGINT DEFAULT NULL,
  preset BIGINT DEFAULT NULL
)
RETURNS stripe_minimal_product.default_price_data_custom_unit_amount
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT ROW(
    enabled, maximum, minimum, preset
  )::stripe_minimal_product.default_price_data_custom_unit_amount;
$$;

ALTER TYPE stripe_minimal_product.default_price_data_recurring
  ADD ATTRIBUTE "interval" TEXT, ADD ATTRIBUTE interval_count BIGINT;

CREATE OR REPLACE FUNCTION stripe_minimal_product.make_default_price_data_recurring(
  "interval" TEXT, interval_count BIGINT DEFAULT NULL
)
RETURNS stripe_minimal_product.default_price_data_recurring
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT ROW(
    "interval", interval_count
  )::stripe_minimal_product.default_price_data_recurring;
$$;

ALTER TYPE stripe_minimal_product.marketing_feature
  ADD ATTRIBUTE "name" TEXT;

CREATE OR REPLACE FUNCTION stripe_minimal_product.make_marketing_feature(
  "name" TEXT
)
RETURNS stripe_minimal_product.marketing_feature
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT ROW("name")::stripe_minimal_product.marketing_feature;
$$;

ALTER TYPE stripe_minimal_product.package_dimension
  ADD ATTRIBUTE height DOUBLE PRECISION,
  ADD ATTRIBUTE "length" DOUBLE PRECISION,
  ADD ATTRIBUTE weight DOUBLE PRECISION,
  ADD ATTRIBUTE width DOUBLE PRECISION;

CREATE OR REPLACE FUNCTION stripe_minimal_product.make_package_dimension(
  height DOUBLE PRECISION,
  "length" DOUBLE PRECISION,
  weight DOUBLE PRECISION,
  width DOUBLE PRECISION
)
RETURNS stripe_minimal_product.package_dimension
LANGUAGE SQL
IMMUTABLE
AS $$
  SELECT ROW(
    height, "length", weight, width
  )::stripe_minimal_product.package_dimension;
$$;

CREATE OR REPLACE FUNCTION stripe_minimal_product._create(
  "name" TEXT,
  "id" TEXT DEFAULT NULL,
  active BOOLEAN DEFAULT NULL,
  default_price_data stripe_minimal_product.default_price_data DEFAULT NULL,
  description TEXT DEFAULT NULL,
  expand TEXT[] DEFAULT NULL,
  images TEXT[] DEFAULT NULL,
  marketing_features stripe_minimal_product.marketing_feature[] DEFAULT NULL,
  metadata JSONB DEFAULT NULL,
  package_dimensions stripe_minimal_product.package_dimension DEFAULT NULL,
  shippable BOOLEAN DEFAULT NULL,
  statement_descriptor TEXT DEFAULT NULL,
...
AS $$
  ...
  response = GD["__stripe_minimal_context__"].client.products.with_raw_response.create(
      name=name,
      id=not_given if id is None else id,
      active=not_given if active is None else active,
      default_price_data=not_given if default_price_data is None else GD["__stripe_minimal_context__"].strip_none(default_price_data),
      ...
      marketing_features=not_given if marketing_features is None else GD["__stripe_minimal_context__"].strip_none(marketing_features),
      metadata=not_given if metadata is None else json.loads(metadata),
      package_dimensions=not_given if package_dimensions is None else GD["__stripe_minimal_context__"].strip_none(package_dimensions),
...
CREATE OR REPLACE FUNCTION stripe_minimal_product.create(
  "name" TEXT,
  "id" TEXT DEFAULT NULL,
  active BOOLEAN DEFAULT NULL,
  default_price_data stripe_minimal_product.default_price_data DEFAULT NULL,
...
AS $$
  BEGIN
    ...
    RETURN jsonb_populate_record(
      NULL::stripe_minimal_product.product,
      stripe_minimal_product._create(
        "name",
        "id",
        active,
        default_price_data,
